...
### Fixed
- **SupabaseTransport** — `Channel`-based connection pool shared across all coroutines in a worker. Pool of 8 pre-warmed keep-alive connections (configurable via `retries` config). Eliminates TLS handshake storms under concurrent load. Failed connections are discarded and replaced automatically.
